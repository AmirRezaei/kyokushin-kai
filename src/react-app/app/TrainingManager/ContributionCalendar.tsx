// File: ./src/app/Equipment/ContributionCalendar.tsx
import {Box, Tooltip, Typography, useTheme} from '@mui/material';
import {addDays, eachDayOfInterval, endOfYear, format, getDay, getMonth, startOfWeek, startOfYear} from 'date-fns';
   ...
   const calendarData = useMemo(() => {
      ...
      contributions.forEach(contribution => {
         ...
         if (daysMap[contributionDate]) {
            ...
            if (contribution.category && !daysMap[contributionDate].categoryIds.includes(contribution.category)) {
            ...
            <>
               <Box sx={{overflowX: 'auto', pb: 2}}>
                  <Box sx={{overflow: 'visible'}}>
                     <Box
                        ...
                        {monthSpans.map(span => (
                           <Tooltip key={`${span.month}-${span.startColumn}`} title={`${span.month}: ${monthlyContributions[span.month] || 0} contributions`} placement='top' arrow>
                              <Typography
                                 variant='caption'
                                 sx={{
                                    gridRow: 1,
               ...
               <Box sx={{mt: 1, display: 'flex', gap: 2, flexWrap: 'wrap'}}>
                  <Box sx={{display: 'flex', alignItems: 'center', gap: 1}}>
                     <Box sx={{width: 14, height: 14, bgcolor: theme.palette.grey[200], borderRadius: 1}} />
                     <Typography variant='caption'>No contributions</Typography>
                  </Box>
                  <Box sx={{display: 'flex', alignItems: 'center', gap: 1}}>
                     <Box sx={{width: 14, height: 14, bgcolor: theme.palette.success.light, borderRadius: 1}} />
                     <Typography variant='caption'>1 contribution</Typography>
                  </Box>
                  <Box sx={{display: 'flex', alignItems: 'center', gap: 1}}>
                     <Box sx={{width: 14, height: 14, bgcolor: theme.palette.success.main, borderRadius: 1}} />
                     <Typography variant='caption'>2 contributions</Typography>
                  </Box>
                  <Box sx={{display: 'flex', alignItems: 'center', gap: 1}}>
                     <Box sx={{width: 14, height: 14, bgcolor: theme.palette.success.dark, borderRadius: 1}} />
                     <Typography variant='caption'>3+ contributions</Typography>
                  </Box>
                  <Box sx={{display: 'flex', alignItems: 'center', gap: 1}}>
                     <Box sx={{width: 14, height: 14, bgcolor: theme.palette.warning.light, borderRadius: 1}} />
                     <Typography variant='caption'>Multiple categories</Typography>
                  </Box>
               </Box>
               <Typography variant='body2' sx={{mt: 1, color: theme.palette.text.secondary}}>
                  Daily contribution activity. Color intensity shows contribution count, special color for multiple categories in a day.
               </Typography>
            </>
         )}
      </Box>
