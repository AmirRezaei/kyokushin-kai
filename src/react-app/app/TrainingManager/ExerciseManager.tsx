...
const ExerciseManager: React.FC = () => {
   ...
   const [currentExercise, setCurrentExercise] = useState<Partial<Exercise>>({equipmentIds: [], muscleGroupIds: []});
   const [batchInput, setBatchInput] = useState<string>('');
   const [batchResults, setBatchResults] = useState<{successful: string[]; failed: {name: string; error: string}[]} | null>(null);
   ...
   const validateSingleExercise = (): boolean => {
      ...
      if (!current.muscleGroupIds || (Array.isArray(current.muscleGroupIds) && current.muscleGroupIds.length === 0)) {
         validationErrors.muscleGroupIds = 'At least one Muscle Group must be selected';
      }

      if (!current.equipmentIds || (Array.isArray(current.equipmentIds) && current.equipmentIds.length === 0)) {
         validationErrors.equipmentIds = 'At least one Equipment must be selected';
      }

      const isDuplicate = exercises.some(ex => ex.name.toLowerCase() === current.name?.trim().toLowerCase() && ex.id !== current.id);
      if (isDuplicate) {
   ...
   const validateAndParseBatchExercises = (): {successful: string[]; failed: {name: string; error: string}[]} => {
      ...
      lines.forEach((line, index) => {
         const [name, muscleGroupsStr, equipmentsStr, how] = line.split('|').map(part => part.trim());
         ...
         const muscleGroupNames = muscleGroupsStr ? muscleGroupsStr.split(',').map(n => n.trim()) : [];
         ...
         if (invalidMuscleGroups.length > 0) {
            failed.push({name, error: `Invalid Muscle Group names: ${invalidMuscleGroups.join(', ')}`});
            return;
         }
   ...
   return (
      <Paper style={{padding: 16, margin: 'auto', maxWidth: 1200}}>
         ...
         <Box sx={{mb: 2}}>
            <Button variant='contained' color='primary' startIcon={<Add />} onClick={handleOpenAddDialog} sx={{mr: 2}}>
               Add New Exercise
            </Button>
               ...
               <DialogContent>
                  <Grid container spacing={2}>
                     <Grid item xs={12} sm={6}>
                        <TextField required label='Exercise Name' name='name' fullWidth value={currentExercise.name || ''} onChange={handleChange} error={Boolean(errors.name)} helperText={errors.name} />
                     </Grid>
                     <Grid item xs={12} sm={6}>
                              ...
                              renderValue={selected => (
                                 <div style={{display: 'flex', flexWrap: 'wrap'}}>
                                    {(selected as string[]).map(value => {
                                       ...
                                       return group ? <Chip key={value} label={group.name} style={{margin: 2}} /> : null;
                                    })}
                                 </div>
                              )}
                     ...
                     <Grid item xs={12} sm={6}>
                        <FormControl fullWidth required error={Boolean(errors.equipmentIds)}>
                           <InputLabel id='equipment-label'>Equipments</InputLabel>
                           <Select
                              labelId='equipment-label'
                              multiple
                              name='equipmentIds'
                              value={currentExercise.equipmentIds || []}
                              ...
                              renderValue={selected => (
                                 <div style={{display: 'flex', flexWrap: 'wrap'}}>
                                    {(selected as string[]).map(value => {
                                       ...
                                       return equipment ? <Chip key={value} label={equipment.name} color='secondary' style={{margin: 2}} /> : null;
                                    })}
                                 </div>
                              )}
                              MenuProps={MenuPropsMultiple}>
                     ...
                     <Grid item xs={12}>
                        <TextField label='How' name='how' fullWidth multiline rows={4} value={currentExercise.how || ''} onChange={handleChange} error={Boolean(errors.how)} helperText={errors.how || 'Optional: Describe how to perform the exercise'} />
                     </Grid>
                  </Grid>
               </DialogContent>
               <DialogActions>
            ...
            <form onSubmit={handleEditExercise} noValidate>
               <DialogContent>
                  <Grid container spacing={2}>
                     ...
                     <Grid item xs={12} sm={6}>
                        <FormControl fullWidth required error={Boolean(errors.equipmentIds)}>
                           <InputLabel id='edit-equipment-label'>Equipments</InputLabel>
                           <Select
                              labelId='edit-equipment-label'
                              multiple
                              name='equipmentIds'
                              value={currentExercise.equipmentIds || []}
                              onChange={handleEquipmentChange}
            ...
            <form onSubmit={handleBatchImport} noValidate>
               <DialogContent>
                  <Typography variant='body2' gutterBottom>
                     Enter exercises, one per line, in the format: Name | Muscle Group Names (comma-separated) | Equipment Names (comma-separated) | How (optional)
                     <br />
                     Example:
                     <br />
                  ...
                  <TextField label='Exercises List' fullWidth multiline rows={6} value={batchInput} onChange={handleBatchInputChange} error={Boolean(errors.batch)} helperText={errors.batch || 'Enter exercises as described above'} />
                  {batchResults && (
                     <Box sx={{mt: 2}}>
                           ...
                           <>
                              ...
                              <List dense>
                                 {batchResults.failed.map(({name, error}) => (
                                    <ListItem key={name}>
                                       <ListItemText primary={name} secondary={error} secondaryTypographyProps={{color: 'error'}} />
                                    </ListItem>
                                 ))}
                              </List>
                           </>
