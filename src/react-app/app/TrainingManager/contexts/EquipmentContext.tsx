...
export const EquipmentProvider: React.FC<EquipmentProviderProps> = ({ children }) => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (res.ok) {
          ...
          console.log('EquipmentContext - Fetched equipment:', data.equipment);
            ...
            const defaults: Equipment[] = [
              {
                id: uuidv4(),
                name: 'Dumbbell',
                description: 'Free weight for isolation exercises.',
              },
              {
                id: uuidv4(),
                name: 'Barbell',
                description: 'Long bar for compound lifts like Squats/Deadlifts.',
              },
              {
                id: uuidv4(),
                name: 'Kettlebell',
                description: 'Ball-shaped weight with a handle.',
              },
              {
                id: uuidv4(),
                name: 'Bench Press',
                description: 'Flat or inclined bench for pressing.',
              },
              {
                id: uuidv4(),
                name: 'Pull-up Bar',
                description: 'Bar for upper body pulling exercises.',
              },
              {
                id: uuidv4(),
                name: 'Treadmill',
  ...
  const addEquipment = async (equipment: Omit<Equipment, 'id'>) => {
    ...
    if (token) {
      try {
        const response = await fetch('/api/v1/gym/equipment', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        ...
        if (!response.ok) {
          ...
          console.error('Failed to add equipment to database:', response.status, errorData);
          alert(
            `Warning: Equipment "${newEquipment.name}" was added locally but failed to save to database. It may disappear on page reload. Error: ${errorData.error || response.statusText}`,
          );
        }
      } catch (e) {
        console.error('Failed to add equipment', e);
        alert(
          `Warning: Equipment "${newEquipment.name}" was added locally but failed to save to database. It may disappear on page reload.`,
        );
      }
    }
  };
  ...
  return (
    <EquipmentContext.Provider
      value={{ equipments, setEquipments, addEquipment, updateEquipment, deleteEquipment }}
    >
      {children}
    </EquipmentContext.Provider>
  );
