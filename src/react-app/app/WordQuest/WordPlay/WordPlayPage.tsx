...
import {
  ...
  getTechniqueWords,
} from '../../../../data/repo/techniqueHelpers';
...
const WordPlayPage: React.FC = () => {
  ...
  const techniqueMap = useMemo(() => {
    ...
    grades.forEach((grade) => {
      map.set(grade.id, grade.techniques);
    });
    return map;
  }, [grades]);

  const allTechniques: TechniqueRecord[] = useMemo(() => {
   ...
   * State Variables
   */
  const [currentTechnique, setCurrentTechnique] = useState<TechniqueRecord | null>(() => {
    if (grades.length > 0) {
  ...
  const [techniquesToShow, setTechniquesToShow] = useState<TechniqueRecord[]>(() => {
    if (grades.length > 0) {
      return techniqueMap.get(grades[0].id) || [];
    }
    return [];
  });
  const [isReviewingChallengingTechniques, setIsReviewingChallengingTechniques] =
  ...
  useEffect(() => {
    ...
    const allWords = [
      ...
      ...getDistractorWords(currentTechnique, distractorCount, previousGradeTechniques),
    ];
    setItems({
   ...
   * Handle Level Change
   */
  const handleLevelChangeByLevelNumber = useCallback(
    (newLevel: number) => {
    ...
    const loadProgress = async () => {
      ...
      if (loadedIds.length > 0) {
        ...
        const knownGradeIds = grades
          .filter(
            (grade) =>
              grade.techniques.length > 0 &&
              grade.techniques.every((tech) => updatedKnownTechniqueIds.has(tech.id)),
          )
          .map((grade) => grade.id);
  ...
  const findContainer = (id: string): 'availableWords' | 'assembledWords' | null => {
    if (id === 'availableWords' || id === 'assembledWords')
      return id as 'availableWords' | 'assembledWords';
  ...
  const isCorrectOrder = useMemo(() => {
    ...
    return arraysAreEqual(items.assembledWords, correctOrder);
  ...
  const reviewChallengingTechniques = () => {
    const challengingTechniqueList = allTechniques.filter((technique) =>
      challengingTechniques.has(technique.id),
    );
  ...
  const handleHint = () => {
    if (!currentTechnique) return;
    const correctOrder = getTechniqueCorrectOrder(currentTechnique);
    const firstMissingWord = correctOrder.find(
      (word) => items.availableWords.includes(word) && !hintedWordIds.includes(word),
    );

    if (firstMissingWord) {
      setHintedWordIds((prev) => [...prev, firstMissingWord]);

      const pointsToDeduct = Math.ceil(10 / correctOrder.length);
  ...
  const progressPercentage = useMemo(() => {
    ...
    return Math.min(
      Math.round(((currentIndex + (progressCompleted ? 1 : 0)) / totalTechniques) * 100),
      100,
    );
    ...
    <Box>
      ...
      <Box sx={{ ml: 1, mr: 1 }}>
        <ProgressBar
          ...
          challengingTechniquesSize={challengingTechniques.size}
          currentIndex={currentIndex}
          techniquesToShowLength={techniquesToShow.length}
        />
        {progressCompleted ? (
          <Box>
            <Typography color="primary" sx={{ mt: 2 }}>
              {isReviewingChallengingTechniques
                ? 'Well done! You have reviewed all challenging techniques.'
                : "Well done! You've completed all techniques in this level."}
            </Typography>
              ...
              <Button
                ...
                onClick={() => {
                  const gradeIndex = grades.findIndex(
                    (grade) => getLevelNumber(grade) === selectedLevel,
                  );
                  if (gradeIndex !== -1) {
                    setIsReviewingChallengingTechniques(false);
          ...
          <Box>
            {attempted &&
              (isCorrectOrder ? (
                <Typography color="success.main">Correct Order! Great job!</Typography>
              ) : (
                <Typography color="error.main">Incorrect Order. Try again!</Typography>
              ))}
            ...
            <Button
              variant="contained"
              color="primary"
              onClick={useRevealAnswer}
              disabled={revealAnswerUsed || !currentTechnique}
              sx={{ mt: 1, mr: 1 }}
            >
            ...
            <Button
              ...
              disabled={
                !currentTechnique ||
                getTechniqueCorrectOrder(currentTechnique).filter((word) =>
                  items.availableWords.includes(word),
                ).length === hintedWordIds.length
              }
              sx={{ mt: 1, mr: 1 }}
            >
              Hint
            ...
            {challengingTechniques.size > 0 && (
              <>
                <Button
                  variant="contained"
                  color="warning"
                  onClick={reviewChallengingTechniques}
                  sx={{ mt: 1 }}
                >
                  Review Challenging Techniques
                </Button>
                <Button
                  variant="contained"
                  color="warning"
                  onClick={showChallengingTechniques}
                  sx={{ mt: 1, ml: 1 }}
                >
        ...
        <LevelSelector
          ...
          knownTechniqueIds={knownTechniqueIds}
        />
      </Box>

      <ChallengingTechniquesModal
        open={openChallengingTechniquesModal}
        onClose={() => setOpenChallengingTechniquesModal(false)}
        challengingTechniques={challengingTechniques}
