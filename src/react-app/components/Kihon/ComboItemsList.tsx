...
import {
  FindGradeByTechniqueId,
  specialTechniqueTagsByCategory,
} from '@/app/Technique/TechniqueData';
...
interface ComboItemsListProps {
  ...
  setComboDifficultyInDialog: React.Dispatch<
...
const ComboItemsList: React.FC<ComboItemsListProps> = ({
  ...
  allCombos,
  setAllCombos,
  setAddTechniqueDialogOpen,
  setSelectedComboIndexForAdding,
  ...
  const duplicateCombo = useCallback(
    (index: number) => {
      ...
      const newCombo = new Combo(
        uuidv4(),
        comboToDup.name + ' (Copy)',
        comboToDup.difficulty,
        comboToDup.tags ? [...comboToDup.tags] : [],
        comboToDup.notes,
        comboToDup.dividerText,
        duplicatedItems,
      );
...
------------------------------------------ */
  ...
  */
  ...
  return (
    <>
      <DraggableList
        items={combos.map((combo) => ({ id: combo.id }))}
            ...
            return (
              ...
              >
                <Box>
                  ...
                  <DraggableList
                    items={combo.items.map((i) => ({ id: i.id }))}
                    onReorder={(newOrder) => handleComboItemsReorder(comboIndex, newOrder)}
                  >
                        ...
                        return (
                          ...
                          <DraggableListItem key={item.id} id={item.id}>
                            ...
                            >
                              {/* Icon & Belt */}
                              {category === 'Kick' && (
                                <KickIcon sx={{ fontSize: '2em', marginRight: '0.5em' }} />
                              )}
                              {category === 'Stand' && (
                                <StandIcon sx={{ fontSize: '2em', marginRight: '0.5em' }} />
                              )}
                              {category === 'Block' && (
                                <BlockIcon sx={{ fontSize: '2em', marginRight: '0.5em' }} />
                              )}
                              {category === 'Strike' && (
                                <StrikeIcon sx={{ fontSize: '2em', marginRight: '0.5em' }} />
                              )}
                              <Box sx={{ width: '5em' }}>
                                <KarateBelt
                                  sx={{ width: '4em', height: '1em' }}
                                  borderRadius="0"
                                  stripes={stripes}
                                  color={beltColorHex}
                                  thickness="0.5em"
                              ...
                              {tech.name.romaji &&
                              tech.name.romaji.toLowerCase().includes('uraken') ? (
                                <Box
                                  sx={{
                                    background: 'smoke',
                                    display: 'flex',
                              ...
                              {item.tags &&
                                item.tags.map((tag, idx) => (
                                  <Chip
                                    ...
                                    size="small"
                                    sx={{ margin: '0.2em' }}
                                    onDelete={() => removeTechniqueTag(comboIndex, itemIndex, tag)}
                                  />
                                ))}
                              ...
                              >
                                ...
                                <IconButton
                                  ...
                                  onClick={() => openTechniqueTagsDialog(comboIndex, itemIndex)}
                                >
                                  <AddIcon />
                                </IconButton>
                                ...
                                <IconButton
                                  ...
                                  onClick={() => duplicateTechnique(comboIndex, itemIndex)}
                                >
                                  <ContentCopyIcon />
                                </IconButton>
                                ...
                                <IconButton
                                  aria-label="remove"
                                  onClick={() => openDeleteDialog(combo.id, item.id)}
                                >
                                  <DeleteIcon />
                                </IconButton>
                              </Box>
                  ...
                  <Box sx={{ marginLeft: '1em', marginTop: '1em' }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                      ...
                      <LinearProgress
                        ...
                        sx={{ width: '20%', height: '1em' }}
                      />
                    </Box>
                    <Typography variant="body1" color="textSecondary" sx={{ marginTop: '0.5em' }}>
                      Techniques:{' '}
                      {
                        combo.items.filter((it): it is TechniqueRef => it instanceof TechniqueRef)
                          .length
                      }
                    </Typography>
                  </Box>
                  ...
                  >
                    ...
                    <Button
                      ...
                      color="primary"
                      startIcon={<AddIcon />}
                      onClick={() => openAddDividerDialog(comboIndex)}
                    >
                      Add Step Divider
                    </Button>
                    <Button
                      ...
                      onClick={() => startPreview(comboIndex)}
                    >
                      Preview
                    </Button>
                    <Button
      ...
      <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)}>
        ...
        <DialogContent>
          {deleteTarget.itemId
            ? 'Are you sure you want to delete this technique/divider?'
      ...
      {/* Add Divider Dialog */}
      <Dialog
        open={addDividerDialogOpen}
        onClose={() => setAddDividerDialogOpen(false)}
        fullWidth
        maxWidth="sm"
      >
      ...
      <Dialog open={previewMode} onClose={closePreview} fullWidth maxWidth="sm">
        ...
        <DialogContent>
          {previewComboIndex !== null && (
            <>
              {allCombos[previewComboIndex].items.map((item) => {
                if (item instanceof TechniqueRef) {
                  const tech = techniqueMap.get(item.techId);
                  if (!tech) return null;
                  return (
        ...
        <DialogActions>
          ...
          <Button
            ...
            disabled={
              previewComboIndex !== null &&
              currentTechniqueIndex === allCombos[previewComboIndex].items.length - 1
            }
          >
            Next
          </Button>
      ...
      {/* Technique Tags Dialog */}
      <Dialog
        open={techniqueTagsDialogOpen}
        onClose={() => setTechniqueTagsDialogOpen(false)}
        fullWidth
        maxWidth="sm"
      >
