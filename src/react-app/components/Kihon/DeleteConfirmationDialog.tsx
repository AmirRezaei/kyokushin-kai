// HEADER-START
// * Project: Kyokushin
// * Path: src/components/DeleteConfirmationDialog.tsx
// ! Purpose: [Action Required: Summarize the component or file purpose.]
//
// ! Tech Stack and Libraries:
// - Core Frameworks: React, Next.js with CSR and not SSR
...
export const DeleteConfirmationDialog: React.FC<DeleteConfirmationDialogProps> = ({open, itemType, storageKey, onDelete, onClose}) => {
   const [skipConfirm, setSkipConfirm] = useState<boolean>(false);

   useEffect(() => {
      const savedSkipConfirm = localStorage.getItem(storageKey) === 'true';
      //setSkipConfirm(savedSkipConfirm);
   }, [storageKey]);
   ...
   }, [open, skipConfirm, onDelete, onClose]);
   ...
   return (
      <Dialog open={open} onClose={onClose} fullWidth maxWidth='sm'>
         ...
         <DialogContent>
            <Typography>Are you sure you want to delete this {itemType}? This action cannot be undone.</Typography>
            <FormControlLabel control={<Checkbox checked={skipConfirm} onChange={handleCheckboxChange} />} label="Don't ask me again" />
         </DialogContent>
         <DialogActions>
