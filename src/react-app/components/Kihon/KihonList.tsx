...
import {Autocomplete, Box, Button, ButtonGroup, Chip, Container, Dialog, DialogActions, DialogContent, DialogTitle, FormControl, InputLabel, ListItemText, MenuItem, Select, SelectChangeEvent, TextField, Typography, useTheme} from '@mui/material';
...
export class Combo {
   ...
   constructor(id: string, name: string, difficulty: 'Beginner' | 'Intermediate' | 'Advanced', tags: string[] = [], notes: string = '', dividerText?: string, items: ComboItem[] = []) {
      this.id = id;
      this.name = name;
      this.items = items;
...
function parseCombosFromJSON(data: unknown): Combo[] {
   ...
   return data.map((comboObj: any) => {
      const c = new Combo(comboObj.id, comboObj.name, comboObj.difficulty, comboObj.tags || [], comboObj.notes || '', comboObj.dividerText, []);
         ...
         for (const itemObj of comboObj.items) {
            if (itemObj && typeof itemObj === 'object') {
               if (itemObj.text && itemObj.id) {
                  ...
                  // TechniqueRef
                  c.items.push(new TechniqueRef(itemObj.id, itemObj.techId, Array.isArray(itemObj.tags) ? [...itemObj.tags] : []));
               }
            }
         }
      }
...
const KihonList: React.FC = React.memo(() => {
  ...
  */
   ...
   const [selectedComboIndexForAdding, setSelectedComboIndexForAdding] = useState<number | null>(null);
   const [newTechniques, setNewTechniques] = useState<string[]>([]);
   const [comboNameInDialog, setComboNameInDialog] = useState<string>('');
   const [comboDifficultyInDialog, setComboDifficultyInDialog] = useState<'Beginner' | 'Intermediate' | 'Advanced'>('Beginner');
      ...
      <Container>
         ...
         <Box marginTop={4}>
            <FormControl fullWidth margin='normal'>
               ...
               <Select value={difficultyFilter} onChange={e => setDifficultyFilter(e.target.value as 'All' | 'Beginner' | 'Intermediate' | 'Advanced')} label='Filter by Difficulty'>
            ...
            <Autocomplete
               ...
               renderInput={params => <TextField {...params} variant='outlined' label='Filter by Tags' placeholder='Select tags' margin='normal' />}
            />
        ...
        */}
         ...
         <ButtonGroup variant='contained' aria-label='Basic button group' sx={{marginTop: 2}}>
            ...
            <input accept='application/json' style={{display: 'none'}} id='import-combos' type='file' onChange={importCombos} />
         ...
         <Dialog
            ...
            fullWidth
            maxWidth='sm'>
            <DialogTitle>{selectedComboIndexForAdding === null ? 'Create New Combo' : 'Edit Combo'}</DialogTitle>
            <DialogContent>
               <TextField autoFocus margin='dense' label='Combo Name' type='text' fullWidth value={comboNameInDialog} onChange={e => setComboNameInDialog(e.target.value)} />
               <FormControl fullWidth margin='normal'>
                  <InputLabel>Difficulty</InputLabel>
                  <Select value={comboDifficultyInDialog} onChange={e => setComboDifficultyInDialog(e.target.value as 'Beginner' | 'Intermediate' | 'Advanced')}>
                     <MenuItem value='Beginner'>Beginner</MenuItem>
                     <MenuItem value='Intermediate'>Intermediate</MenuItem>
                     <MenuItem value='Advanced'>Advanced</MenuItem>
                  </Select>
               ...
               <Autocomplete
                  ...
                  renderTags={(value: string[], getTagProps) => value.map((option: string, index: number) => <Chip variant='outlined' label={option} {...getTagProps({index})} />)}
                  renderInput={params => <TextField {...params} variant='outlined' label='Combo Tags' />}
               />

               <TextField margin='dense' label='Notes' type='text' fullWidth multiline rows={4} value={comboNotesInDialog} onChange={e => setComboNotesInDialog(e.target.value)} />

               {/* Technique selection by type */}
               {TechniqueTypes.map(typeStr => {
                  const type = typeStr as TechniqueType;
                  const techniquesForType = allTechniques.filter(t => t.type === type);
                  const selectedForType = newTechniques
                     .filter(id => {
                        const tech = techniqueMap.get(id);
                        return tech?.type === type;
                     })
                     .map(id => techniqueMap.get(id)?.romaji || '');
                              ...
                              const chosenIDs = vals
                                 .map(jap => {
                                    for (const [tid, technique] of techniqueMap) {
                                       if (technique.romaji === jap && technique.type === type) return tid;
                                    }
                                    return '';
                                 })
                                 .filter(id => id !== '');
                  ...
                  onClick={() => {
                     ...
                     if (selectedComboIndexForAdding !== null) {
                        ...
                        for (const tid of newTechniques) {
                           updatedCombos[selectedComboIndexForAdding].items.push(new TechniqueRef(uuidv4(), tid));
                        }
                     } else {
                        // Create new combo
         ...
         <Dialog open={renameDialogOpen} onClose={closeRenameDialog} fullWidth maxWidth='sm'>
            ...
            <DialogContent>
               <TextField autoFocus margin='dense' label='Combo Name' type='text' fullWidth value={newComboName} onChange={e => setNewComboName(e.target.value)} />
            </DialogContent>
            <DialogActions>
