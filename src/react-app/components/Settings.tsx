...
import {
  exportLocalStorageDataAsJSON,
  importLocalStorageDataFromJSON,
} from './ImportExportLocalStorage';
...
const Settings: React.FC = () => {
  const [history, setHistory] = useState<GradeHistoryEntry[]>(() =>
    SettingsManager.getGradeHistory(),
  );
  ...
  const [trainedDaysInput, setTrainedDaysInput] = useState<string>(() =>
    SettingsManager.getTrainedDays().toString(),
  );
              ...
              >
                {grades
                  .filter(
                    (grade) =>
                      editingIndex !== null &&
                      (grade.id === history[editingIndex].gradeId ||
                        !history.some((h) => h.gradeId === grade.id)),
                  )
                  .map((grade) => (
                    <MenuItem key={grade.id} value={grade.id}>
                      <Box display="flex" alignItems="center">
      ...
      <Grid2 container spacing={2} alignItems="center">
        ...
        <Grid2 size={{ xs: 6, sm: 6, md: 6, lg: 6, xl: 6 }}>
          <Box display="flex" gap={1}>
            <TextField
              type="number"
              value={trainedDaysInput}
              onChange={(e) => setTrainedDaysInput(e.target.value)}
              inputProps={{ min: 0 }}
              size="small"
              fullWidth
            />
      ...
      <Grid2 container spacing={2}>
        ...
        <Grid2 size={{ xs: 6, sm: 6, md: 6, lg: 6, xl: 6 }}>
          <Button
            ...
            onClick={exportLocalStorageDataAsJSON}
          >
            Export
          </Button>
        </Grid2>
