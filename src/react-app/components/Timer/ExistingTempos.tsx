// HEADER-START
// * Project: Kyokushin
// * Path: src/components/Timer/ExistingTempos.tsx
...
export const ExistingTempos: React.FC<ExistingTemposProps> = ({tempos, currentTempo, onTempoChange, onDeleteTempo}) => {
   return (
      <Paper elevation={3} sx={{p: 3}}>
         ...
         <Box>
            {tempos.map(tempo => (
               <Box key={tempo.id} display='flex' justifyContent='space-between' alignItems='center' mb={1}>
                  <Button variant={tempo.bpm === currentTempo ? 'outlined' : 'contained'} onClick={() => onTempoChange(tempo.bpm)} sx={{flexGrow: 1, marginRight: 1}}>
                     {tempo.label} ({tempo.bpm} BPM)
                  </Button>
                  <IconButton color='error' onClick={() => onDeleteTempo(tempo.id)}>
                     <DeleteIcon />
                  </IconButton>
               </Box>
            ))}
