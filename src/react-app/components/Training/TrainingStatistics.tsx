...
const TrainingStatistics: React.FC<TrainingStatisticsProps> = React.memo(({sessions}) => {
   ...
   const statistics = useMemo(() => {
      ...
      const totalDuration = sortedSessions.reduce((sum, session) => sum + (Number(session.duration) || 0), 0);
      ...
      const totalDurationByIntensity = sortedSessions.reduce<Record<string, number>>((acc, session) => {
      ...
      const totalIntensity = sortedSessions.reduce((sum, session) => sum + (intensityMap[session.intensity] || 0), 0);
      const averageIntensity = totalSessions > 0 ? (totalIntensity / totalSessions).toFixed(2) : 'N/A';

      // Most Common Type
      const mostCommonTypeEntry = Object.entries(sessionsByType).reduce((a, b) => (a[1] > b[1] ? a : b), ['', 0]);
      const mostCommonType = mostCommonTypeEntry[0] || 'N/A';

      // Most Common Intensity
      const mostCommonIntensityEntry = Object.entries(sessionsByIntensity).reduce((a, b) => (a[1] > b[1] ? a : b), ['', 0]);
