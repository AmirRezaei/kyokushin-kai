...
const TrainingTypeBreakdownChart: React.FC<TrainingTypeBreakdownChartProps> = React.memo(({sessions}) => {
   ...
   const maxYValue = useMemo(() => {
      return aggregatedData.reduce((max, dataPoint) => Math.max(max, dataPoint.y), 0);
   }, [aggregatedData]);
      ...
      <Box>
         ...
         <Box sx={{display: 'flex', gap: 2, mb: 2}}>
            <FormControl variant='outlined' size='small'>
               ...
               <Select labelId='time-scale-label' value={timeScale} onChange={handleTimeScaleChange} label='Time Scale'>
            ...
            <FormControl variant='outlined' size='small'>
               ...
               <Select labelId='num-months-label' value={numMonths} onChange={handleNumMonthsChange} label='Latest Months'>
                  {[3, 6, 12, 24].map(month => (
         ...
         <Box sx={{height: 400}}>
            <BarChart series={series} colors={['#1976d2']} />
         </Box>
      </Box>
   );
});
