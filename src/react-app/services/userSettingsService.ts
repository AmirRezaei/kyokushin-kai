...
// NEW: Version key
const SETTINGS_VERSION_KEY = 'user.settings.version';
...
export const getLocalSettingsVersion = (): number => {
  if (typeof window === 'undefined') return 0;
  const v = window.localStorage.getItem(SETTINGS_VERSION_KEY);
  return v ? parseInt(v, 10) : 0;
};

export const applyPersistedSettingsToLocalStorage = (
  ...
  window.localStorage.setItem(
    SKIP_COMBO_ITEMS_KEY,
    String(Boolean(settings.skipDeleteConfirmForComboItems)),
  );
  window.localStorage.setItem(SKIP_COMBO_KEY, String(Boolean(settings.skipDeleteConfirmForCombo)));

  if (typeof version === 'number') {
    window.localStorage.setItem(SETTINGS_VERSION_KEY, version.toString());
  }
};

export const fetchUserSettingsFromServer = async (
  ...
  try {
    ...
    if (typeof data.version === 'number') {
      if (typeof window !== 'undefined') {
        window.localStorage.setItem(SETTINGS_VERSION_KEY, data.version.toString());
      }
    }
...
export const pushLocalSettingsToServer = async (tokenOverride?: string): Promise<boolean> => {
  ...
  const expectedVersion = getLocalSettingsVersion();
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 409) {
        console.warn('Settings conflict detected. Reloading latest.');
        if (typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent('settings-conflict'));
        }
        await hydrateSettingsFromServer(token);
        return false;
      }
...
export const hydrateSettingsFromServer = async (
  tokenOverride?: string,
): Promise<PersistedSettings | null> => {
...
const sanitizeSettings = (settings: PersistedSettings): PersistedSettings => {
  return {
    ...
    lastTrainedDate:
      typeof settings.lastTrainedDate === 'string' && settings.lastTrainedDate.length > 0
        ? settings.lastTrainedDate
        : null,
